  ...
  return (
    <div>
        <input 
            ...
            className='placeholder:italic placeholder:text-slate-400 text-lg bg-white w-full border border-slate-300 rounded-md pt-2 pl-2 shadow-sm focus:outline-none focus:border-slate-500 focus:ring-sky-500 focus:ring-1'
        />
        <ul className="bg-white/80">
            {searchTerm !== "" && ListOfCafes.filter((cafe: CafeInfo) => cafe.name.toLowerCase().includes(searchTerm)
            ).map((cafe: CafeInfo) => (
                <li key={cafe.name} className="text-xl py-3 pl-2">{cafe.name}</li>
            ))}
        </ul>
    </div>
  )
